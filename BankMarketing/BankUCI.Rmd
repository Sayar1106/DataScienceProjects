...
bank<-subset(x = bank, select = -c(duration,default))
...
up_train <- upSample(x = train_data[,-ncol(train_data)], y = train_data$y)                         
...
pred <- prediction(predictions = knn_test_preds[,2], labels = test_data$y)
